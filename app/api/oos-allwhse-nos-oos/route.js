/**
 * GET /api/oos-allwhse-nos-oos
 *
 * Pulls the OOS vendor report from Snowflake (replacing the per-vendor CSV
 * upload in the OOS Tracker). Returns ALL rows as JSON; the front end filters by VENDOR_NAME (vendor tab), so this returns everything.
 *
 ...
 *
 * Table is configurable via env (SNOWFLAKE_OOS_ALLWHSE_TABLE) so it can be repointed
 * from a personal schema to a governed one later without a code change.
 * Default: PERSONAL.TIMOTHY.OOS_ALLWHSENOSOOS
 *
...
var OOS_TABLE = process.env.SNOWFLAKE_OOS_ALLWHSE_TABLE || "PERSONAL.TIMOTHY.OOS_ALLWHSENOSOOS";
