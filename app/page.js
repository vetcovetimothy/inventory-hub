    ...
    {subPage === "data" && <div>
      ...
      {data.length > 0 ? <div>
        {Object.keys(dismissed).length > 0 && <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8, padding: "8px 14px", background: "rgba(100,116,139,0.06)", borderRadius: 8 }}>
          <span style={{ fontSize: 12, color: "#6B7280" }}>{Object.keys(dismissed).length} item{Object.keys(dismissed).length > 1 ? "s" : ""} dismissed</span>
          <button onClick={function() { setDismissed({}); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#3B82F6", fontWeight: 500 }}>Show all</button>
        </div>}
