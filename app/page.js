...
function WHT(props) {
  ...
  var _dismissed = useState([]), dismissedItems = _dismissed[0], setDismissedItems = _dismissed[1];
  ...
  var fetchData = useCallback(function() {
    ...
    (async function() {
      try {
        ...
        setData(rows); setRunBy(who); setRunTime(now); setLoading(false); setSubPage("data"); setShipNotes(carried); setDismissedItems([]); persist(rows, false, who, now, carried); toast(cfg.label + ": Fetched " + rows.length + " lines");
      } catch (err) {
        setLoading(false);
        toast("Error: " + err.message, "error");
      }
